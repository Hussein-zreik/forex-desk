...
async def composite(symbol: str = Query("XAU=F"), db: AsyncSession = Depends(get_db)) -> dict:
    ...
    The computation lives in services/bias.py so the snapshot job (poller) can
    share it — see that module for the per-signal rationale.
    """
    return await bias_service.compute_composite(db, symbol)
