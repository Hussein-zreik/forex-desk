...
async def composite(symbol: str = Query("XAU=F"), db: AsyncSession = Depends(get_db)) -> dict:
    """Gold-directional composite read aggregating six macro/technical signals.

    Each signal scores positive = bullish for gold; inverted where a rising value
    hurts gold (dollar, real yields). Every signal is best-effort so one upstream
    failure never sinks the widget (it just reads neutral).
    """
    signals: list[dict] = []
