...
| Date | Decision / Note |
|------|-----------------|
| 2026-06-24 | 🎨 **Reworked light mode via `ui-ux-pro-max` (was flat all-white).** Ran the skill's `--domain color` → recommended a **slate** light palette (bg `#F8FAFC`, text slate-800 `#1E293B`, border slate-200 `#E2E8F0`, glass cards ≥ white/80, muted slate-600). Applied to the light `:root` in `index.css`, keeping our indigo brand: cool slate canvas (`#edf0f6` + slate-200 depth), **white-alpha `--surface` so cards pop** as white panels, slate-900 text (`#0f172a`), slate-600 muted (`#475569`), visible slate borders, cooler shadows + a soft indigo ambient wash. Also gave Welcome feature icon tiles a `bg-primary/10` indigo tint (avoids white-on-white in light, nicer in both modes). Verified via Playwright (landing + dashboard in light): cards now lift off the canvas with depth; dark mode untouched (only the light `:root` changed). 21 tests, lint, build green. |
