...
| Date | Decision / Note |
|------|-----------------|
| 2026-06-26 | 📊 **Forex-expert content expansion ("do all" from the review backlog).** Added: **Risk/Position-size calculator** (`lib/risk.ts` pure pip-value/lot-sizing/R:R + config-aware `RiskCalcWidget`, fully offline, 7 unit tests); **per-widget context** (`useWidgetData.updatedAt` → "updated Xs ago" on every data widget via `fmtAgo`; Pivots/Key-Levels distance-from-price + nearest-level highlight); **COT/CFTC positioning** (`services/cot.py` Socrata API + `/api/cot` + `COTWidget`, config-aware, 2 tests); **news expansion** (`FX_FEEDS` ForexLive/FXStreet/DailyFX/FinancialJuice/Investing, `tag_currencies`, `aggregate` dedupe; `/api/news?feed=gold|fx`; `NewsWidget` feed prop + currency-tag chips + "FX News" widget); **Central Bank rates** static dashboard (`lib/centralBanks.ts` + `CBRatesWidget`, dated/indicative); **retail sentiment** (`services/sentiment.parse_retail` + `/api/retail-sentiment` + `RetailSentimentWidget`, contrarian read — needs a data source wired, returns `unavailable` until then); **calendar currency filter** on the Calendar page. New widget categories: Tools (risk), Positioning (COT, retail). **56 frontend tests, 47 backend tests, lint + tsc clean, build passes.** Verified via Playwright: risk calc + CB rates render with content, no console errors (data-backed widgets show `unavailable` under the env's egress block, as expected). ⚠️ Follow-ups: retail-sentiment + rate-expectations(implied probabilities) need a real/auth data source; CB rates are static and need periodic updates. |
| 2026-06-25 | 🧩 **Dashboard grid improvements (post-review).** (#2) Clamp widget `minW` to the breakpoint column count in `buildLayouts`/`addWidget` + a render-time guard in `DashboardGrid` (legacy/server data) — fixes minW 3/4 widgets overflowing the 2-col `xxs` (phone) grid. (#1) **Multi-instance widgets:** decoupled `WidgetInstance.id` from `type` (unique `type-uuid` ids), added optional per-instance `config` (symbol/title) threaded `registry render(ctx, config)` → widget; `addWidget(type, config?)` now allows duplicates; 7 indicator widgets (bias/pivots/volatility/mtf/hilo/keyLevels/smc) are `configurable` (a `symbols` list) and the AddWidgetMenu shows a symbol picker — so e.g. Composite Bias for Gold + EUR/USD + GBP/USD coexist. (#3) **`reconcileLayouts`** on load: drops orphan layout items, appends first-fit slots for missing widgets, ensures all breakpoints exist, clamps w/minW; unknown widget types filtered. (#4) **Empty-grid `EmptyState`** with an "add your first widget" CTA. Verified via Playwright: 3 distinct bias instances render; empty state appears after removing all widgets; no console errors. **45 frontend tests, lint + tsc clean, build passes.** |
| 2026-06-25 | 🧱 **Built a production-grade reusable UI component library + migrated the whole app onto it.** Adopted **class-variance-authority** (user's choice) and refactored the existing primitives (Button +`loading`, Input +`invalid`, Badge +variants) to cva with unchanged public APIs. New primitives/composites in `frontend/src/components/ui/` (barrel `index.ts`): **`AsyncBoundary`** (the core reuse win — one declarative loading→error(retry)→empty→data ladder; stale-while-revalidate), **`AsyncWidget`** (WidgetFrame + AsyncBoundary), `Field` (a11y label/aria/error wiring), `Select`, `Spinner`, `Skeleton`, `EmptyState`, `ErrorState`, `DataTable` (responsive: real table ≥md, stacks to label/value cards <md via `useMediaQuery`; keeps `useVirtualRows` windowing), `SegmentedControl` (radiogroup + arrow-key nav), `StatCard`, `PageHeader`. **Full migration:** all 21 data-driven dashboard widgets → AsyncWidget (fixed QuoteCard silent-catch + Correlation/etc. missing empty states); Portfolio/Journal → StatCard + Field forms (real labels + inline validation) + responsive DataTable; Calendar/Learning → SegmentedControl; AuthForm → Field + Button loading. **Docs:** `docs/design-system/components.md` (architecture, props/API tables, usage, best practices). **42 frontend tests (+21), lint clean, tsc clean, build passes** (main bundle ~244 KB, code-split intact). Static/WebSocket-only widgets left as-is (no fetch state to standardize). Branch `claude/markdown-previous-session-40t2mz`. |
| 2026-06-24 | 🎨 **Rebuilt the welcome page via `ui-ux-pro-max` (user disliked the old one).** Ran the skill's `search.py --design-system` + `--domain landing` → recommended the **Hero → social-proof → Testimonials → CTA** pattern (3–5 testimonials w/ name/role/photo, social proof *before* CTA) + glassmorphism. Kept our **Linear/Modern indigo + near-black theme** (overrode the skill's amber/violet default per the user's "colors matching the theme"). New `Welcome.tsx`: animated glass "live signal" hero card (draw-in sparkline via framer-motion `pathLength`, easing bias needle, ticking price, floating profit-factor chip), animated aurora glow, pulsing live badge, on-view **count-up stat counters**, live ticker, 4 feature cards, **6 testimonials w/ star ratings + avatars + roles ("4.8 avg")**, CTA band, footer — all reveals reduced-motion-safe. Verified via Playwright at 1440px + 390px: no horizontal scroll, contrast/touch-targets per the skill checklist. 21 tests, lint, build green. |
